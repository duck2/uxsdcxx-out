...
const char *gtok_lookup_t_connection_boxes[] = {"connection_box"};
enum class atok_t_connection_boxes {X_DIM, Y_DIM, NUM_BOXES};
const char *atok_lookup_t_connection_boxes[] = {"x_dim", "y_dim", "num_boxes"};
...
const char *gtok_lookup_t_rr_graph[] = {"binary_nodes_and_edges", "connection_boxes", "channels", "switches", "segments", "block_types", "grid", "rr_nodes", "rr_edges"};
...
inline atok_t_bin_nodes alex_t_bin_nodes(const char *in){
	unsigned int len = strlen(in);
	switch(len){
	case 4:
		switch(*((triehash_uu32*)&in[0])){
...
inline atok_t_connection_box alex_t_connection_box(const char *in){
	unsigned int len = strlen(in);
	switch(len){
	case 2:
		switch(in[0]){
...
inline gtok_t_connection_boxes glex_t_connection_boxes(const char *in){
	unsigned int len = strlen(in);
	switch(len){
	case 14:
		switch(*((triehash_uu64*)&in[0])){
...
inline atok_t_connection_boxes alex_t_connection_boxes(const char *in){
	unsigned int len = strlen(in);
	switch(len){
	case 5:
		switch(*((triehash_uu32*)&in[0])){
...
void load_timing(const pugi::xml_node &root, t_timing *out){
	...
	std::bitset<5> test_astate = astate | std::bitset<5>(0b11111);
...
void load_pin(const pugi::xml_node &root, t_pin *out){
	out->value = string_pool.add(root.child_value());
	if(errno != 0)
...
void load_meta(const pugi::xml_node &root, t_meta *out){
	...
	for(pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()){
		...
		case atok_t_meta::NAME:
			out->name = string_pool.add(attr.value());
			if(errno != 0)
...
void load_node_loc(const pugi::xml_node &root, t_node_loc *out){
	...
	std::bitset<6> test_astate = astate | std::bitset<6>(0b010000);
...
void load_connection_box(const pugi::xml_node &root, t_connection_box *out){
	...
	std::bitset<2> astate = 0;
	for(pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()){
		atok_t_connection_box in = alex_t_connection_box(attr.name());
		if(astate[(int)in] == 0) astate[(int)in] = 1;
...
int gstate_t_connection_boxes[2][1] = {
	{0},
	{0},
};
void load_connection_boxes(const pugi::xml_node &root, t_connection_boxes *out){
	int next, state=1;
	for(pugi::xml_node node = root.first_child(); node; node = node.next_sibling()){
		gtok_t_connection_boxes in = glex_t_connection_boxes(node.name());
	...
	std::bitset<3> astate = 0;
	for(pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()){
		atok_t_connection_boxes in = alex_t_connection_boxes(attr.name());
		if(astate[(int)in] == 0) astate[(int)in] = 1;
		...
		case atok_t_connection_boxes::NUM_BOXES:
			out->num_boxes = std::strtol(attr.value(), NULL, 10);
			if(errno != 0)
				throw std::runtime_error("Invalid value `" + std::string(attr.value()) + "` to load a int into out->num_boxes.");
			break;
		default: break; /* Not possible. */
		}
	}
...
void load_node(const pugi::xml_node &root, t_node *out){
	...
	std::bitset<4> test_astate = astate | std::bitset<4>(0b0100);
...
void load_rr_graph(const pugi::xml_node &root, t_rr_graph *out){
	...
	for(pugi::xml_attribute attr = root.first_attribute(); attr; attr = attr.next_attribute()){
		...
		case atok_t_rr_graph::TOOL_NAME:
			out->tool_name = string_pool.add(attr.value());
			if(errno != 0)
		...
		case atok_t_rr_graph::TOOL_VERSION:
			out->tool_version = string_pool.add(attr.value());
			if(errno != 0)
		...
		case atok_t_rr_graph::TOOL_COMMENT:
			out->tool_comment = string_pool.add(attr.value());
			if(errno != 0)
